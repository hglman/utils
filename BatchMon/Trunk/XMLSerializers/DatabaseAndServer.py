# XML Parser/Data Access Object DatabaseAndServer.py
"""AUTO-GENERATED Source file for DatabaseAndServer.py"""
import xml.sax
import Queue
...
class row_class(API.xml.base_xml.XMLNode):
    def __init__(self, attrs):
        self.level = 2
        self.path = [None, u'DatabaseAndServer']
        API.xml.base_xml.XMLNode.__init__(self, "row", attrs, None, [])

class DatabaseAndServer_class(API.xml.base_xml.XMLNode):
    def __init__(self, attrs):
        self.level = 1
        self.path = [None]
        self.row = []
        API.xml.base_xml.XMLNode.__init__(self, "DatabaseAndServer", attrs, None, [])
...
class NodeHandler(xml.sax.handler.ContentHandler):
    ...
    def startElement(self, name, attrs): # creating the node along the path being tracked
        ...
        if name == "":
            ...
            self.obj_depth.append(row_class(p_attrs))

        elif name == "DatabaseAndServer":
            self.obj_depth.append(DatabaseAndServer_class(p_attrs))
    ...
    def endElement(self, name): # need to append the node that is closing in the right place
        ...
        if name == "":
            ...
            self.obj_depth[-2].row.append(self.obj_depth[-1]) #  make this object a child of the next object up...
